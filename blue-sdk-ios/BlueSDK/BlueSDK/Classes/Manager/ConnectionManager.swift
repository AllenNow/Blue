...
final class ConnectionManager {
    ...
    private(set) var state: ConnectionState = .disconnected {
        didSet {
            ...
            logger.info("State changed: \(oldValue) → \(state)")
    ...
    func connect(peripheral: CBPeripheral) {
        guard state == .disconnected else {
            logger.warn("Cannot connect in state \(state)")
            return
        }
    ...
    private func scheduleConnectTimeout() {
        ...
        connectTimeoutTimer = Timer.scheduledTimer(withTimeInterval: ConnectionManager.connectTimeout, repeats: false) { [weak self] _ in
            ...
            self.logger.error("Connection timeout (\(Int(ConnectionManager.connectTimeout))s), aborting")
    ...
    private func startReconnect() {
        guard reconnectAttempts < ConnectionManager.maxReconnectAttempts else {
            logger.warn("Max reconnect attempts (\(ConnectionManager.maxReconnectAttempts)) reached")
            transitionTo(.disconnected)
            CallbackDispatcher.shared.dispatch { [weak self] in
        ...
        logger.info("Reconnect attempt \(reconnectAttempts), retrying in \(delay)s")
    ...
    func cancelReconnection() {
        ...
        logger.info("Reconnection cancelled manually")
        cancelReconnect()
        reconnectAttempts = 0
        transitionTo(.disconnected)
    }
