...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `fixedAuthKey` | String? | null | 固定认证密钥（4位hex如"05FA"），null则自动计算 |
| `logLevel` | LogLevel | DEBUG | 日志级别：DEBUG/INFO/WARN/ERROR/NONE |
| `autoAuthEnabled` | Boolean | true | 连接成功后是否自动认证 |
| `autoReconnect` | Boolean | true | 意外断线后是否自动重连 |
| `maxReconnectAttempts` | Int | 5 | 最大自动重连次数 |
| `language` | BlueSDKLanguage | SYSTEM | 错误描述语言：SYSTEM/ZH/EN/DE |
| `customPhoneMac` | String? | null | 自定义手机标识（12位hex），用于密钥计算 |
| `rawFrameLogEnabled` | Boolean | false | 是否输出 BLE 帧原始十六进制数据（调试用） |
...
| 添加观察者 | `addObserver(observer)` | `addObserver(_:)` | 多页面同时监听 |
| 移除观察者 | `removeObserver(observer)` | `removeObserver(_:)` | |
| UUID直连 | — | `connect(byIdentifier:completion:)` | iOS: 已绑定设备快速重连 |
| 切换语言 | `setLanguage(language)` | `setLanguage(_:)` | 运行时切换错误描述语言 |
| 密钥展示 | `currentAuthKeyDisplay` (属性) | `currentAuthKeyDisplay` (属性) | 当前使用的认证密钥 |
